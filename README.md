...
AIHR 是一个基于 Frappe HR 二次开发的 AI 化招聘 MVP。

当前版本聚焦国内公司的招聘核心流程，目标是先把岗位需求、简历导入、AI 初筛、面试协同和 Offer 管理从 Excel 与人工搬运中收口到一个系统里，并用 AI 减少摘要、整理、提醒和记录这些重复工作。

## 当前范围

当前 MVP 主链路：

`岗位需求 -> 招聘中岗位 -> ZIP 简历导入 -> AI 初筛 -> 经理复核 -> 面试协同 -> 面试反馈 -> Offer`

当前更适合这样理解：

- 这是一个招聘优先的 AIHR MVP，不是完整 HRMS
- 当前已经可以本地试用和演示招聘主链路
- 员工主档、入职交接、薪酬建档与 Payroll 正式联动仍在后续范围内
...
- 当前界面已收口为招聘核心工作流，默认突出岗位、导入、初筛、面试和 Offer
