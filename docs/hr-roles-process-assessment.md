...
1. `HR / 面试官 / 用人经理` 的页面入口已经分成 3 个中心，但 **Workspace 仍是公开的**，没有按角色真正收口。
2. 当前系统 **没有启用任何 Workflow**，所以岗位审批、Offer 审批、录用确认还不是制度化审批流。
3. `用人经理` 作为业务角色在产品里已经存在，但在权限层 **还没有独立角色定义**，目前更接近“概念角色”，不是“正式系统角色”。
4. 当前最值得优先补的是：`岗位需求权限收口 + 经理角色落地 + 审批流启用 + 导入结果页`。
...
| 用人经理 / 部门经理 | 提岗位需求、复核 AI 摘要、面试决策、Offer 决策 | **未正式落地** | 新增 `AIHR Hiring Manager` |
...
- `Job Requisition`
  - 当前只有 `System Manager` 有读写创建权限
  - 这和“部门经理提需求，HR补充岗位信息”的业务目标不一致
- `Job Opening`
  - 当前只有 `HR User` 有权限
  - 用人经理虽然有工作台入口，但不具备正式的岗位查看/复核收口方案
- `Job Applicant`
  - 当前主要由 `HR User` 操作
  - 用人经理没有正式的候选人访问权限设计
- `AIHR` 三个工作台
  - 目前 `roles` 字段都是空数组
  - 说明它们是公开 Workspace，而不是按角色可见
...
- `Job Requisition` 只有 `System Manager` 可创建

这说明：

- “部门经理提岗位需求”这件事，逻辑上支持，权限上未落地
...
- 新增角色：`AIHR Hiring Manager`
- 给 3 个 AIHR Workspace 显式绑定角色，而不是公开
- 修正 `Job Requisition` 的创建权限，让经理可发起
- 让经理中心默认只能看“我负责/我部门”的岗位和候选人
